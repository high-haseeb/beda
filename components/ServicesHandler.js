...
function ServicesHandler() {
  return (
    <div className='lg:w-[800px] w-auto h-auto bg-white/95 absolute top-8 lg:right-0 right-10 z-50 rounded-xl '>
        <HowCanWeHelp/>
    </div>
  )
}
...
const NavSection=({imgSrc, title, detail,ID})=>{
    return (
        <Link href={`/services#${ID}`} className={`flex flex-col items-center justify-center text-center lg:p-8 p-4 gap-4 text-pretty`}>
